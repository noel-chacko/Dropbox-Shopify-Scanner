#!/usr/bin/env python3
"""
Auto scanner router - Handles 1-2 roll orders automatically, manual for large orders.

WORKFLOW:
1. Set order once
2. Scans upload automatically (1-2 scans per order)
3. After 2 scans for current order, asks for next order
4. Perfect for high-volume single/double roll processing

Handles 99% automatically, manual input only for new orders.
"""
...
scan_count_for_order = 0
pending_scans = []  # Scans waiting for new order
auto_lock = threading.Lock()
pending_scan_event = threading.Event()
...
# =================== SET ORDER ===================
def set_next_order():
    global current_order_data, scan_count_for_order
    
    while True:
        s = input("\n🔍 Enter NEXT order number (or 'q' to quit): ").strip()
        ...
        if pick.isdigit():
            ...
            if 1 <= idx <= len(results):
                ...
                with auto_lock:
                    ...
                    scan_count_for_order = 0
                print(f"✅ Current order: #{r['name']} ({email}) - Ready for 1-2 scans")
                pending_scan_event.set()
                return

# =================== AUTO UPLOAD ===================
def auto_upload_scan(twin_dir: Path, date_str: str, twin_check: str):
    with auto_lock:
        if current_order_data is None:
            pending_scans.append((twin_dir, date_str, twin_check))
            print(f"⏳ {date_str}/{twin_check} - WAITING for order assignment")
            return
        
        # Auto-upload if we have an order
        if current_order_data["mode"] == "stage":
            dest = f"{DROPBOX_ROOT}/_staging/{date_str}/{twin_check}/photos"
            print(f"\n📤 Auto-uploading {date_str}/{twin_check} → STAGING")
        else:
            customer_root, _ = get_or_create_customer_root(current_order_data["order_node"])
            safe_order = current_order_data["order_no"].replace("#", "").replace("/", "_")
            dest = f"{customer_root}/{safe_order}/{twin_check}"
            print(f"\n📤 Auto-uploading {date_str}/{twin_check} → Order #{current_order_data['order_no']}")
        
        uploaded = upload_folder(twin_dir, dest)
        print(f"✓ Uploaded {uploaded} files to {dest}")
        
        scan_count_for_order += 1
        
        # Check if we need a new order (after 1 scan wait 3 seconds to see if another comes)
        if scan_count_for_order == 1:
            print(f"  📊 1 scan uploaded for Order #{current_order_data['order_no']}")
            print(f"  ⏳ Waiting 3 seconds to see if another scan comes...")
            time.sleep(3)
            
            # If no new scan came after 3 seconds, clear for next order
            if scan_count_for_order == 1:  # Still only 1 scan
                print(f"\n{'='*60}")
                print(f"✅ Completed 1 scan for Order #{current_order_data['order_no']}")
                print(f"{'='*60}")
                current_order_data = None
                scan_count_for_order = 0
                pending_scan_event.set()
        elif scan_count_for_order >= 2:
            print(f"\n{'='*60}")
            print(f"✅ Completed {scan_count_for_order} scans for Order #{current_order_data['order_no']}")
            print(f"{'='*60}")
            current_order_data = None
            scan_count_for_order = 0
            pending_scan_event.set()
    ...
    def _scan_tree(self):
        ...
        for date_dir in sorted([d for d in root.iterdir() if d.is_dir()]):
            for twin_dir in sorted([d for d in date_dir.iterdir() if d.is_dir()]):
                ...
                auto_upload_scan(twin_dir, date_dir.name, twin_dir.name)
...
def main():
    print(f"\n{'='*60}")
    print("🚀 AUTO SCANNER ROUTER - High-Volume Mode")
    print(f"{'='*60}")
    print(f"Watching: {NORITSU_ROOT}")
    print("="*60)
    print("\n💡 HOW IT WORKS:")
    print("   1. Enter an order number")
    print("   2. Scans upload automatically (1-2 scans)")
    print("   3. Program asks for NEXT order")
    print("   4. Repeat - handles 1-2 roll orders automatically!")
    print("="*60)
    ...
    def order_manager():
        while True:
            pending_scan_event.wait()  # Wait for signal
            pending_scan_event.clear()
